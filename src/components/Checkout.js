...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    let address = true;
    let selected = true;
    if (getTotalCartValue(items) > parseInt(localStorage.getItem("balance"))) {
      // console.log("1");
      balance = false;
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
    ...
    if (balance === true && address === true && selected === true) {
      // console.log("4");
  ...
  const selectAddress = (address) => {
    ...
    setAddresses({ ...addresses, selected: address.address });
  };

  const performCheckout = async (token, items, addresses) => {
    let request = validateRequest(items, addresses);
    // console.log(request, "123");
    let addressId = addresses.all.find(
      (address) => address.address === addresses.selected
    );
    if (request) {
      try {
        let response = await axios.post(
  ...
  useEffect(async () => {
    if (localStorage.getItem("token") === null) {
      enqueueSnackbar("Must be logged in to acces checkout", {
        variant: "warning",
      });
      history.push("/login");
    }
            ...
            <Box>
              {addresses.all.length > 0 ? (
                addresses.all.map((address) => (
                  ...
                  >
                    <Box onClick={() => selectAddress(address)}>
                      {address.address}
                    </Box>
                    <Box color="#008000">
                      <Button
